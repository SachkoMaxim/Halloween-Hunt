...
public class SeekBehavior : SteeringBehavior
{
    ...
    [SerializeField] private float targetRechedThreshold = 0f;
    ...
    public override (float[] danger, float[] interest) GetSteering(float[] danger, float[] interest, EnemyAI enemyAI)
    {
        ...
        {
            if (enemyAI.targets == null || enemyAI.targets.Count <= 0)
            {
            ...
            else
            {
                ...
                enemyAI.currentTarget = enemyAI.targets.OrderBy(
                    target => Vector2.Distance(target.position, transform.position)
                ).FirstOrDefault();
            }

        }

        if (enemyAI.currentTarget != null && enemyAI.targets != null && enemyAI.targets.Contains(enemyAI.currentTarget))
        {
            targetPositionCached = enemyAI.currentTarget.position;
        }
